...
import { getParticipants, addParticipants, updateParticipant, deleteParticipants } from '#/server/participants.ts';
...
export default function ManageParticipants({
  ...
  const [newRole, setNewRole] = useState<'ADMIN' | 'COUNTER' | 'PARTICIPANT'>('PARTICIPANT');
  ...
  const filtered = search
    ? displayParticipants.filter((p) => p.email.toLowerCase().includes(search.toLowerCase()))
    : displayParticipants;

  return (
    <div className="space-y-6">
      <div>
        <h3 className="mb-3 text-lg font-semibold text-foreground">Legg til deltaker</h3>
        <div className="flex gap-2">
          <Input
            ...
            className="flex-1"
          />
          <Select value={newRole} onValueChange={(v) => setNewRole(v as 'ADMIN' | 'COUNTER' | 'PARTICIPANT')}>
      ...
      <div>
        <h3 className="mb-3 text-lg font-semibold text-foreground">Last opp CSV</h3>
        <Textarea
          rows={4}
        ...
        <Button type="button" variant="outline" className="mt-2" onClick={handleCSVUpload} disabled={!csvText.trim()}>
          Last opp
        </Button>
      </div>
      ...
      <div>
        <div className="mb-3 flex items-center gap-3">
          <h3 className="text-lg font-semibold text-foreground">Deltakere ({displayParticipants.length})</h3>
          <Input placeholder="Sok..." value={search} onChange={(e) => setSearch(e.target.value)} className="max-w-xs" />
          ...
          {filtered.map((p) => (
            <div key={p.id} className="flex items-center gap-3 rounded-lg border bg-card p-3">
              ...
              <div className="flex-1">
                ...
                {!p.isParticipant && <span className="text-xs text-muted-foreground">Invitert</span>}
              </div>
              {!isLocal && p.isParticipant ? (
                <>
                  <Select
                    ...
                    onValueChange={(role) =>
                      updateMutation.mutate({
                        participantId: p.id,
                        role: role as 'ADMIN' | 'COUNTER' | 'PARTICIPANT',
                      })
                    }
                  >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-xs text-muted-foreground">Stemmerett</span>
                  </div>
                </>
              ) : (
                <span className="text-xs font-medium text-muted-foreground">
                  {p.role === 'ADMIN' ? 'Admin' : p.role === 'COUNTER' ? 'Teller' : 'Deltaker'}
                </span>
              )}
            </div>
          ))}
          {filtered.length === 0 && (
            <p className="py-4 text-center text-sm text-muted-foreground">Ingen deltakere enna.</p>
          )}
        </div>
      </div>
    </div>
