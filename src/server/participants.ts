    ...
    for (const p of data.participants) {
      ...
      if (existingUser) {
        ...
        const [existing] = await db
          .select()
          .from(participant)
          .where(and(eq(participant.userId, existingUser.id), eq(participant.meetingId, data.meetingId)));
        ...
        const [existing] = await db
          .select()
          .from(invite)
          .where(and(eq(invite.email, p.email), eq(invite.meetingId, data.meetingId)));
...
export const updateParticipant = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    if (data.isVotingEligible !== undefined) updates.isVotingEligible = data.isVotingEligible;
...
export const deleteParticipants = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    for (const pid of data.participantIds) {
      const [p] = await db.select().from(participant).where(eq(participant.id, pid));
    ...
    if (data.inviteEmails) {
      for (const email of data.inviteEmails) {
        await db.delete(invite).where(and(eq(invite.email, email), eq(invite.meetingId, data.meetingId)));
      }
    }
...
export const registerAsParticipant = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    if (!m.allowSelfRegistration) throw new Error('Selvregistrering er ikke aktivert');
