...
export const updateVotations = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    for (const v of data.votations) {
      ...
      const [updatedVotation] = await db.update(votation).set(updateData).where(eq(votation.id, id)).returning();

      if (alts) {
        // Delete existing alternatives and recreate
        await db.delete(alternative).where(eq(alternative.votationId, id));
...
export const updateVotationIndexes = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    for (const v of data.votations) {
      await db.update(votation).set({ index: v.index }).where(eq(votation.id, v.id));
    }
