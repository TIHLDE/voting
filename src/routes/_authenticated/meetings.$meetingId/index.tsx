...
function MeetingLobby() {
  ...
  const isAdmin = meeting?.participants?.some(
...
function SelfRegistrationPanel({
    ...
    return (
      <div className="rounded-xl border bg-card p-6 text-center shadow-sm">
        <p className="text-muted-foreground">
          Selvregistrering er ikke aktivert for dette motet. Du kan aktivere
          det i moteinnstillingene.
        </p>
      </div>
    )
  }
  ...
  const regUrl =
    typeof window !== 'undefined'
      ? `${window.location.origin}/meetings/${meetingId}/register`
      : ''
  ...
  return (
    <div className="rounded-xl border bg-card p-6 shadow-sm">
      ...
      <p className="mb-4 text-sm text-muted-foreground">
        Del denne lenken eller QR-koden med deltakere som skal registrere seg
        selv.
      </p>
