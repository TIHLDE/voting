...
export const castStvVote = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    await db.transaction(async (tx) => {
      ...
      const [newStvVote] = await tx.insert(stvVote).values({ votationId: data.votationId }).returning();
...
export const startNextVotation = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const openVotation = await db.query.votation.findFirst({
      where: and(eq(votation.meetingId, data.meetingId), eq(votation.status, 'OPEN')),
    });
    if (openVotation) {
    ...
    const next = await db.query.votation.findFirst({
      where: and(eq(votation.meetingId, data.meetingId), eq(votation.status, 'UPCOMING')),
    ...
    await db.update(votation).set({ status: 'OPEN' }).where(eq(votation.id, next.id));
...
export const updateVotationStatus = createServerFn({ method: 'POST' })
  .inputValidator(
    z.object({
      votationId: z.string(),
      status: z.enum(['UPCOMING', 'OPEN', 'CHECKING_RESULT', 'PUBLISHED_RESULT', 'INVALID']),
    }),
  )
  .handler(async ({ data }) => {
    ...
    if (!validateStatusTransition(v.status, data.status)) {
      throw new Error(`Ugyldig statusovergang fra ${v.status} til ${data.status}`);
    }
    ...
    await db.update(votation).set({ status: data.status }).where(eq(votation.id, data.votationId));
...
export const getMyReview = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    const [review] = await db
      ...
      .where(and(eq(votationResultReview.votationId, data.votationId), eq(votationResultReview.participantId, p.id)));
