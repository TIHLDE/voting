...
export const getVotationResults = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    // Check visibility
    if (v.hiddenVotes && p.role !== 'ADMIN' && p.role !== 'COUNTER' && v.status !== 'PUBLISHED_RESULT') {
...
export const getWinnerOfVotation = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    const winners = await db.query.alternative.findMany({
      where: and(eq(alternative.votationId, data.votationId), eq(alternative.isWinner, true)),
    });
