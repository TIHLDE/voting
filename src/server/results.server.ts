...
async function computeSimpleResult(votationId: string) {
  ...
  if (winners.length === 1) {
    await db.update(alternative).set({ isWinner: true }).where(eq(alternative.id, winners[0].id));
  }
}
...
async function computeQualifiedResult(votationId: string, threshold: number, eligibleCount: number) {
  const alts = await db.query.alternative.findMany({
    where: eq(alternative.votationId, votationId),
    with: { votes: true },
  });
  ...
  for (const alt of alts) {
    if (alt.votes.length > requiredVotes) {
      await db.update(alternative).set({ isWinner: true }).where(eq(alternative.id, alt.id));
    }
  }
}
...
async function computeStvResult(votationId: string, numberOfWinners: number, resultId: string) {
  ...
  await db.update(votationResult).set({ quota }).where(eq(votationResult.votationId, votationId));
  ...
  while (winners.size < numberOfWinners) {
    const remaining = alts.filter((a) => !eliminated.has(a.id) && !winners.has(a.id));
    ...
    if (remaining.length <= numberOfWinners - winners.size) {
      ...
      await saveRound(
        ...
        eliminated,
        winners,
        Array.from(winners).filter((id) => remaining.some((a) => a.id === id)),
        [],
      );
      break;
    }
    ...
    for (const ballot of ballots) {
      const topChoice = ballot.rankings.find((r) => !eliminated.has(r.alternativeId) && !winners.has(r.alternativeId));
      if (topChoice) {
        voteCounts.set(topChoice.alternativeId, (voteCounts.get(topChoice.alternativeId) ?? 0) + ballot.weight);
      }
    }
        ...
        if (surplus > 0) {
          ...
          for (const ballot of ballots) {
            const topChoice = ballot.rankings.find(
              (r) => !eliminated.has(r.alternativeId) && !winners.has(r.alternativeId),
            );
            const currentTop = ballot.rankings.find((r) => r.alternativeId === altId);
    ...
    if (roundWinners.length === 0) {
      ...
      if (remainingAfterElim < seatsLeft) {
        ...
        for (let i = 0; i < canEliminate && i < lowestAlts.length; i++) {
    ...
    await saveRound(resultId, round, alts, ballots, eliminated, winners, roundWinners, roundLosers);
  ...
  for (const winnerId of winners) {
    await db.update(alternative).set({ isWinner: true }).where(eq(alternative.id, winnerId));
  }
}
...
export async function setWinner(votationId: string) {
  ...
  const [eligibleResult] = await db
    .select({ count: count() })
    .from(participant)
    .where(and(eq(participant.meetingId, v.meetingId), eq(participant.isVotingEligible, true)));
  ...
  switch (v.type) {
    case 'SIMPLE':
      ...
      break;
    case 'QUALIFIED':
      await computeQualifiedResult(votationId, v.majorityThreshold, eligibleResult.count);
      break;
    case 'STV':
      await computeStvResult(votationId, v.numberOfWinners, result.votationId);
      break;
  }
}
